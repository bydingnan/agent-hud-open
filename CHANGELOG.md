...
- OMP sessions that use `cursor/*` (or any other model) now report live turns to the HUD: the Pi session observer also installs under `~/.omp/agent` as `agent-hud-session.ts` when `agent-hud.ts` is already the attention observer, and the HUD reads sessions/turns from both `~/.pi/agent` and `~/.omp/agent`. The observer resolves the turns directory from the extension's own agent home when `PI_CODING_AGENT_DIR` is unset (OMP's usual case). `agent_settled` records a completion unless the last stop reason was `error` / `aborted` / `length` (OMP often settles after `toolUse` rather than `stop`). Turn snapshots are parsed ahead of large transcripts, and the open-agent file watch includes every turns directory. Cursor lifecycle hooks still apply only to Cursor Agent / IDE — an OMP process is not `cursor-agent`.
