...
- A host that relays alerts elsewhere passes `onIslandEvents`. It receives every check after the island has presented it, including checks that found nothing, with the report and time the check used; the host maps that update and never runs a second tracker or presents again. The standalone application is one such host: its `SystemNotifier` turns the completions and interruptions of every check into system notifications.
- Completions and interruptions in an update already honor `Settings.liveStatusEnabled(for:)`; hosts apply the same preference in any other relay or synchronization service they add ([session lifecycle](session-lifecycle.md)). An interruption is a turn that was in flight and stopped without finishing — an interrupt, an abort, a failed attempt — named only when the client itself recorded the stop, so a turn a quiet source parked never reads as one.
...
| `IslandEventTracker.Update` | AgentHUDCore | `completions` (new, Live status on, oldest first), `interruptions` (in-flight turns that stopped without finishing, same gating), `quotaAlerts` (warnings, exhaustion, resets), `exhaustedWindows` and `criticalWindows` (threshold crossings with the reading that crossed; reaching zero supersedes critical in the same reading), `resetCreditGrants` (accounts whose Codex reset-credit count rose, with the added credits when the provider lists them) |
