...
| `--permission-hook claude\|qoder\|qoderCN\|qoderWork` | The handler the client invokes when it is about to ask whether a tool may run: forwards the payload to the running application and waits, prints back the decision the user gave, and exits 0. Prints nothing — leaving the client's own permission flow untouched — when the application is not running, the payload cannot be read, or nobody answers before the client stops waiting. It never initializes the interface or queries an account. |
