...
- A finished turn and an interrupted one both say so in Notification Center. The island still shows each event where the screen can see it; a system notification also says it when the user cannot, and macOS keeps the permission and the quiet hours. The app asks once, on first launch of a bundled build; a denied or unbundled run stays silent and the island keeps working.
