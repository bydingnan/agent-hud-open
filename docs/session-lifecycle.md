...
| Pi / OMP | Yes, with the observer | Yes, with the observer | `agent_start`, `agent_settled` and `session_shutdown` from the Agent HUD extension under `~/.pi/agent` and `~/.omp/agent` (OMP may use `agent-hud-session.ts` when `agent-hud.ts` is already the attention observer). `ask` and tool-approval requests publish `waitingForApproval` (opening a turn when Ask arrives with no active run, as Cursor-via-OMP often does) and hold the turn across `agent_end` until the wait clears; the HUD also reads `<agent-home>/agent-hud/attention` written by OMP’s attention observer, and will reopen a turn that `agent_end` already marked completed while that file is still present. When that attention observer is installed, deleting the inbox file releases an OMP turn still marked waiting — presence and absence of the file are both authoritative. An assistant stop alone does not finish a run. Using a `cursor/*` model inside OMP is still an OMP/Pi turn — it does not fire Cursor lifecycle hooks. |
...
- The island announces each completed turn once, and each turn that starts waiting for the user once, for clients whose Live status is on (`IslandEventTracker`). Completions that happened before the application started are history, not events, and turns that finished — or waits that opened — while Live status was off are not replayed when it is turned back on. The standalone app also posts those events to Notification Center.
...
- The standalone host installs or updates its extension under the Pi directory (`PI_CODING_AGENT_DIR`, default `~/.pi/agent`) and, when present, under `~/.omp/agent`. OMP already uses `agent-hud.ts` for attention prompts, so session turns install as `agent-hud-session.ts` there. Existing Pi/OMP processes need one `/reload`; new ones load automatically. A same-named file that is not Agent HUD's session observer is left alone. The HUD reads sessions, turn snapshots and attention inboxes from both agent homes.
...
| OMP / Pi attention inbox | `Sources/AgentHUDCore/Providers/OpenAgents/OpenAgentAttention.swift` |
